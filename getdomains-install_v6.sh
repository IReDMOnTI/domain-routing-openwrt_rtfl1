...
add_mark() {
	...
	grep -q "99 vpn_v6" /etc/iproute2/rt_tables || echo '99 vpn_v6' >> /etc/iproute2/rt_tables
    
    if ! uci show network | grep -q mark0x2; then
        printf "\033[32;1mConfigure mark v6 rule\033[0m\n"
        uci add network rule6
        uci set network.@rule[-1].name='mark0x2'
        uci set network.@rule[-1].mark='0x2'
        uci set network.@rule[-1].priority='100'
        uci set network.@rule[-1].lookup='vpn_v6'
        uci commit
    fi
}
...
add_set() {
	...
	if uci show firewall | grep -q "@ipset.*name='vpn_domains_v6'"; then
        printf "\033[32;1mSet v6 already exist\033[0m\n"
		else
        printf "\033[32;1mCreate set v6\033[0m\n"
        uci add firewall ipset
        uci set firewall.@ipset[-1].name='vpn_domains_v6'
        uci set firewall.@ipset[-1].match='dst_net'
	...
	if uci show firewall | grep -q "@rule.*name='mark_domains_v6'"; then
        printf "\033[32;1mRule for set v6 already exist\033[0m\n"
    else
        printf "\033[32;1mCreate rule set v6\033[0m\n"
        uci add firewall rule
        uci set firewall.@rule[-1]=rule
        uci set firewall.@rule[-1].name='mark_domains_v6'
        uci set firewall.@rule[-1].src='lan'
        uci set firewall.@rule[-1].dest='*'
        uci set firewall.@rule[-1].proto='all'
        uci set firewall.@rule[-1].ipset='vpn_domains_v6'
